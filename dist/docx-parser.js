...
function extract(el, parentDom) {
  ...
  try {
    for (var _iterator = el.children[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      switch (item.tagName) {
        case 'p':
          var para = new _Paragraph2.default(item, prop);
          var p = para.getDom();
...
var _BaseTag2 = __webpack_require__(5);

var _BaseTag3 = _interopRequireDefault(_BaseTag2);
...
var Paragraph = function (_BaseTag) {
  _inherits(Paragraph, _BaseTag);
  ...
  _createClass(Paragraph, [{
    ...
    value: function getDom() {
      ...
      if (this.prop) {
        // deal numbering
        var i = this.prop.getNumbering(this.docNode);
        if (i) {
          var pre = document.createElement('pre');
          pre.appendChild(document.createTextNode(i));
          p.appendChild(pre);
